...
public class Ex37{
	public static void start(){
		MapPerformance37.start();
	}
}

class MapPerformance37 extends MapPerformance{
	public static void start() {
...
class SimpleHashMap37<K, V> extends AbstractMap<K, V> {
	...
	public V put(K key, V value) {
		...
		while (it.hasNext()) {
			...
			if (iPair.getKey().equals(key)) {
				...
				found = true;
				System.out.println(
						"Collision at " + iPair + ": " + probes + " probe" + ((probes == 1) ? "" : "s") + " needed");
				break;
			}
		}
		if (!found)
	...
	public boolean containsValue(Object value) {
		for (ArrayList<MapEntry<K, V>> bl : buckets) {
			if (bl == null)
				continue;
